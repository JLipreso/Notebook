...
14. **TEACHER DESIGN PASSES OPENED (D-033 — Q-M2-1 answered "run now").** Briefs 4 (teacher mobile) + 5 (teacher browser) are written and ready in [2026-09-13-006-Brand-Colors/](../../documents/2026-09-13-006-Brand-Colors/README.md); the Lead Developer runs them in Claude Design (pass 4 first), exports land in the same folder with completion notes, pass table updated per canvas. Remaining M2 questions Q-M2-2…4 still await `**Decision:**` lines; Q-M3-2/Q-M3-3 await the boss/client.
15. **M1 BUILD TRACKER (boss-facing, live) — https://claude.ai/code/artifact/74514cdd-95cb-44b5-8170-b7277b5fa991** (2026-09-13): the 12 phases with live statuses from the artifact's shared database (`phases/phase-001…012`, seeded `pending`). Read for everyone who can open it; status changes need edit access (click the chip: Pending → In progress → Done) or a Claude session `write_db` on `phases/phase-NNN` `{status, updated_at}`. The plan README's completion ritual (step 3) now includes flipping it. **Lead Developer to-dos: share the link with the boss (view) and grant the junior edit access.** Note: the tracker declares the db capability, so it is organization-internal — viewers must be signed in.
