...
| 5 — M2 TEACHER browser (12 screens) | [claude-design-brief-5.md](claude-design-brief-5.md) | `Notebook-Teacher-Browser-12-screens.html` (expected) | **ready to run** — use the pass-4 canvas as reference |

**Typefaces (confirmed from the approved canvases, folded back 2026-09-13): display = Fraunces · UI/body = Figtree.** Set in the preset's `fontFamily` tokens and loaded via `@fontsource` imports in each app's `main.ts` — bundled with the build (offline-safe), never from a CDN. The M1 design thread is closed; **M2 teacher passes 4–5 are open (D-033)** and must complete before M2 Phase-201. Tablet / admin passes stay parked (D-029; admin = M3 pre-work).
